...
import { IconSearch, IconEye, IconEdit, IconTrash } from '../AdminComponents/AdminSidebarIcons'
    ...
    {/* Header Section */}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden text-sm md:text-base">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-3 p-5 border-b border-slate-200 bg-slate-50/50">
          <div className="flex items-center bg-white border border-slate-300 rounded-lg px-3 py-2 w-full sm:w-75 focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-500 transition duration-200">
            ...
            <tbody className="divide-y divide-slate-100">
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-5">
                    <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold uppercase ${statusStyles[u.status]}`}>
                      {u.status}
                    </span>
                  </td>
