...
export default function ProjectDetail() {
  ...
  const fetchProject = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/projects/${id}?t=${Date.now()}`)
      setProject(res.data);
      setError(null);
    } catch (err) {
  ...
  const openApplication = () => {
    const firstOpenRole = project?.roles?.find(r => r.filled < (r.max || 1));
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#F1F0E8] font-bold text-[#89A8B2]">Loading...</div>;
  if (error) return <div className="min-h-screen flex items-center justify-center bg-[#F1F0E8] text-rose-500 font-bold">{error}</div>;
  if (!project) return null;

  const isOwner = project.creator?._id === currentUserId;
  const alreadyApplied = project.applications?.some(app => app.user?._id === currentUserId || app.user === currentUserId);
    ...
    <div className="min-h-screen flex flex-col bg-[#F1F0E8] font-sans text-slate-800 pt-20">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-8 py-10">
        <div className="mb-6 flex gap-2 text-sm font-bold text-slate-500">
          <Link to="/projects" className="hover:text-[#89A8B2]">Projects</Link>
          <span>/</span>
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-8">
            <div className="max-w-2xl">
              <div className="flex gap-3 mb-4">
                <span className="text-[10px] font-bold uppercase tracking-widest bg-[#89A8B2]/20 text-[#89A8B2] ring-1 ring-[#89A8B2]/30 px-3 py-1 rounded-full">{project.status}</span>
                <span className="text-[10px] font-bold uppercase tracking-widest bg-[#F1F0E8] text-slate-500 border border-[#B3C8CF]/30 px-3 py-1 rounded-full">{project.applicantsCount || 0} Applicants</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-slate-800 mb-4">{project.title}</h1>
              <p className="text-lg text-slate-600 font-medium mb-6">
                {project.description?.substring(0, 160) || "No description provided."}...
              </p>
              ...
              <div className="flex items-center gap-4 border-t border-[#B3C8CF]/30 pt-6">
                ...
                <div>
                   <h4 className="font-bold text-slate-800 text-sm">Created by {project.creator?.name}</h4>
                   <p className="text-xs text-slate-500 font-semibold">{new Date(project.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
                </div>
              </div>
            </div>
               ...
               {!isOwner && (
                 <button 
                  ...
                  >
                   ...
                   {!alreadyApplied && <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>}
                 </button>
               )}
               <button className="bg-white border-2 border-[#B3C8CF] text-slate-700 px-8 py-3.5 rounded-2xl font-bold hover:border-[#89A8B2] transition">Save Project</button>
            </div>
          </div>
        </div>
                ...
                {project.roles?.map((role, i) => {
                  ...
                  return (
                    <div key={i} 
                      ...
                      onClick={() => { if(isAvailable && !isOwner && !alreadyApplied) { setSelectedRole(role.title); setIsApplyModalOpen(true); } }}
                    >
                      <div className="flex justify-between items-start mb-2">
                         <h4 className="font-bold text-slate-800">{role.title}</h4>
                         {isAvailable ? <span className="w-2 h-2 rounded-full bg-[#89A8B2] mt-1.5 shadow-[0_0_8px_rgba(137,168,178,0.8)]"></span> : <span className="text-xs bg-slate-200 text-slate-500 px-2 py-0.5 rounded-lg font-bold">Filled</span>}
                      </div>
                      ...
                      <div className="mt-3 flex justify-between items-center text-[10px] font-bold text-slate-400">
                        <span>{role.filled}/{role.count || 1} filled</span>
                        {isAvailable && !isOwner && !alreadyApplied && <span className="text-[#89A8B2] group-hover:underline">Click to apply →</span>}
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="fixed inset-0 bg-[#F1F0E8]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#E5E1DA] w-full max-w-lg rounded-[2rem] border border-[#B3C8CF]/50 shadow-2xl overflow-hidden">
            <div className="p-8 border-b border-[#B3C8CF]/30 bg-[#F1F0E8]/50 flex justify-between items-start">
               ...
               <button onClick={() => setIsApplyModalOpen(false)} className="w-8 h-8 flex items-center justify-center bg-white rounded-full text-slate-400 hover:text-rose-500 transition shadow-sm border border-[#B3C8CF]/30 font-bold">✕</button>
            </div>
