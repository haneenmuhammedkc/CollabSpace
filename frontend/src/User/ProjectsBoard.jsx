        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
          <div>
            ...
            <p className="text-slate-600 font-medium text-lg max-w-2xl">
              Discover exciting open-source initiatives to join, or pitch your own idea and build your dream team.
            </p>
          </div>
          <button 
          ...
          <div className="flex gap-2 w-full lg:w-auto overflow-x-auto scrollbar-none pb-2 lg:pb-0">
            {['All', 'Frontend', 'Backend', 'Full Stack', 'Mobile', 'AI/ML'].map(f => (
              <button 
                ...
                className={`px-5 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${
                  activeFilter === f 
                    ? 'bg-[#89A8B2] text-white shadow-sm' 
                    : 'bg-transparent text-slate-500 hover:bg-[#B3C8CF]/20 hover:text-slate-700'
                }`}
              >
                {f}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {projects.map((p) => (
            <div
              ...
              className="flex flex-col bg-[#E5E1DA] p-8 border border-[#B3C8CF]/30 rounded-3xl hover:border-[#89A8B2] hover:shadow-xl hover:-translate-y-2 transition-all duration-500 cursor-pointer group relative"
            >
              ...
              <div className="relative z-10 flex-grow flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full ${
                    p.status === 'Recruiting' 
                      ? 'bg-[#89A8B2]/20 text-[#384a51] ring-1 ring-[#89A8B2]/30 text-[#89A8B2]' 
                ...
                <h4 className="text-xl font-bold text-slate-800 group-hover:text-[#5e7780] transition-colors duration-300 leading-snug mb-2">
                  {p.title}
                </h4>

                <p className="text-sm text-slate-500 font-semibold mb-6">
                  Posted by <span className="text-slate-700">{p.author}</span>
                </p>
                ...
                <div className="mb-6 flex-grow">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {p.roles?.map((r, idx) => (
                      <span key={idx} className="text-xs font-bold text-[#5e7780] bg-white border border-[#B3C8CF]/40 px-3 py-1 rounded-full shadow-sm">
                        {r}
                      </span>
                    ))}
                  </div>
                ...
                <div className="border-t border-[#B3C8CF]/30 pt-4 flex gap-2 w-full overflow-hidden">
                  {p.tags?.map((tag, idx) => (
                    <span key={idx} className="text-[10px] font-bold text-[#89A8B2] bg-[#F1F0E8] px-2 py-1 rounded-md">
                      {tag}
                    </span>
                  ))}
                </div>
